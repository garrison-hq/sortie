...
- `packages/core` — SDK + engine + CLI: `contracts.ts` (single source of truth for all
  ...
  `cli.ts` (bin `sortie`: extract | agent | search | fetch | query | profile | batch | runs).
...
- `pnpm --filter @garrison-hq/sortie-ui e2e` — full-stack e2e (rebuilds, boots real server, one live LLM call)
...
- Deployment target is a remote Docker host (docker compose); never bind hardcoded
  `localhost` in server code — use `SORTIE_HOST`/`SORTIE_PORT`.
