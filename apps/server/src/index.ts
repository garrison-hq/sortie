...
import { createRunQueue, createRunStore } from '@nanofish/core';
...
function log(message: string): void {
  process.stderr.write(`[nanofish-server] ${message}\n`);
}
...
function parsePort(value: string | undefined): number {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    log(`invalid NANOFISH_PORT "${value}" — using ${DEFAULT_PORT}`);
    return DEFAULT_PORT;
  }
  return port;
}
...
async function main(): Promise<void> {
  ...
  const host = process.env.NANOFISH_HOST ?? DEFAULT_HOST;
  const port = parsePort(process.env.NANOFISH_PORT);
  const dataDir = resolve(process.env.NANOFISH_DATA_DIR ?? './data');
